    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow-card overflow-hidden">
        {isLoading ? (
          ...
          categories.map((cat) => (
            <div key={cat.id} className="flex items-center justify-between px-6 py-4 border-b border-surface-200 last:border-0">
              {editing === cat.id ? (
                <input defaultValue={cat.name} onBlur={(e) => { update.mutate({ id: cat.id, data: { name: e.target.value } }); setEditing(null); }}
                  autoFocus className="flex-1 px-2 py-1 text-sm border border-brand-500 rounded focus:outline-none" />
              ) : (
                <div>
                  <p className="text-sm font-medium text-text-primary">{cat.name}</p>
                  {cat.description && <p className="text-xs text-text-muted">{cat.description}</p>}
                </div>
              )}
              <div className="flex items-center gap-3 ml-4">
                <span className="text-xs text-text-muted">{cat.product_count || 0} products</span>
                <button onClick={() => setEditing(cat.id)} className="p-1.5 hover:bg-surface-100 rounded text-text-muted">
