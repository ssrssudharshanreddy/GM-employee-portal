        ...
        {isLoading ? (
          ...
          items.map((n) => (
            <div
              ...
              className={`p-4 flex gap-3 cursor-pointer hover:bg-surface-50 ${!n.read_at ? 'bg-blue-50/50' : ''}`}
            >
              <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${n.read_at ? 'bg-surface-200' : 'bg-brand-600'}`} />
              <div className="min-w-0">
                <p className={`text-sm ${n.read_at ? 'text-text-secondary' : 'font-medium text-text-primary'}`}>{n.title}</p>
